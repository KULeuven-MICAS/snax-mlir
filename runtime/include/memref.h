...
struct OneDMemrefF64 {
  double *data; // allocated pointer: Pointer to data buffer as allocated,
                // only used for deallocating the memref
  double *aligned_data; // aligned pointer: Pointer to properly aligned data
                        // that memref indexes
  intptr_t offset;
  intptr_t shape[1];
  intptr_t stride[1];
...
typedef struct OneDMemrefF64 OneDMemrefF64_t;
