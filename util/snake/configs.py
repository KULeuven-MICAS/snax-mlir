...
def get_snax_gemmx_1d_config(snax_mlir_path: str | None = None) -> dict[str, Any]:
    # use CONDA_PREFIX to access pixi env
    snax_utils_path = os.environ["CONDA_PREFIX"] + "/snax-utils"
    snitch_sw_path = snax_utils_path + "/snax-kul-cluster-dse-1d"
    config: dict[str, Any] = {}
    config.update(get_default_paths())
    config.update(get_default_snax_paths())
    config.update(get_default_flags(snitch_sw_path, snax_mlir_path=snax_mlir_path))
    ...
    config["num_harts"] = 2
    config["vltsim"] = (
        snax_utils_path + "/snax-kul-cluster-dse-1d-rtl/bin/snitch_cluster.vlt"
    )
    return config
