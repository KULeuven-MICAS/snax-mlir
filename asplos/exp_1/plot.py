...
# === Plotting helper ===
def plot_system(df_in: pd.DataFrame, system_label: str, out_path: str, title: str):
    sns.set(style="whitegrid")

    df_sys = df_in[df_in["System"] == system_label].copy()
    # Reverse operation order for better appearance in hbar
    df_sys["Operation"] = pd.Categorical(
        df_sys["Operation"],
        categories=sorted(df_sys["Operation"].unique(), reverse=True),
        ordered=True,
    )

    plt.figure(figsize=(4, 3))
    ax = sns.barplot(data=df_sys, y="Operation", x="OPS/cycle", orient="h", color="#073763ff")
    ...
    # Explicit tick values (adjust as needed)
    tick_vals = [20, 30, 50, 100, 200, 300, 400, 500]
    ax.set_xticks(tick_vals)
    ax.set_xticklabels([str(v) for v in tick_vals])
    ...
    # Labels/title
    plt.xticks(rotation=45)
    ax.set_xlabel("Effective OPS/cycle", fontsize=15)
    ax.set_ylabel("")
    ax.set_yticklabels([])
    # ax.set_title(title)
    ...
    # Add annotations
    for container in ax.containers:
        ax.bar_label(container, fmt="%.1f", label_type="center", fontsize=11, color="white", fontweight="bold")

    # No legend needed (single system per figure)
...
plot_system(df, "MatMul", "plot_matmul.png", "Effective OPS/cycle - MatMul")
plot_system(df, "MatVec", "plot_matvec.png", "Effective OPS/cycle - MatVec")
