    ...
    def match_and_rewrite(self, linalg_op: linalg.Generic, rewriter: PatternRewriter):
        ...
        if library_call == "snax_gemm" or library_call == "snax_gemm_stream" or library_call == "snax_gemmx_stream":
            ...
            tsl_input_a = TiledStridedLayoutAttr(
                TiledStridedLayout(
                    [
                        TiledStride([Stride(256 * k // 8 if k else None, m // 8 if m else None), Stride(8, 8)]),
            ...
            tsl_input_b = TiledStridedLayoutAttr(
                TiledStridedLayout(
                    [
                        TiledStride([Stride(256, k // 8 if k else None), Stride(1, 8)]),
                        TiledStride([Stride(256 * k // 8 if k else None, n // 8 if n else None), Stride(8, 8)]),
                    ],
                    # offset=64,
                )
            )
            ...
            # insert layout_cast ops
            new_input_a = LayoutCast.from_type_and_target_layout(linalg_op.inputs[0], tsl_input_a)

            new_input_b = LayoutCast.from_type_and_target_layout(linalg_op.inputs[1], tsl_input_b)
