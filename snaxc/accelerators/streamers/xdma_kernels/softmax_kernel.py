...
class SoftMaxXDMA(XDMAKernel):
    ...
    def set_stride_patterns(
        ...
        input_index = None
        input_bound_list = [
            bound.data for bound in input_snax_stride_pattern.upper_bounds
        ]
        ...
        ), "Input stride pattern must contain vector length"

        # Bring the last upper bound and temporal stride to the front
        input_snax_stride_pattern = StridePattern(
            [
                bound.data
                for i, bound in enumerate(input_snax_stride_pattern.upper_bounds)
                if i == input_index
            ]
            + [
                bound.data
            ...
            [
                stride.data
                for i, stride in enumerate(input_snax_stride_pattern.temporal_strides)
                if i == input_index
            ]
            + [
                stride.data
        ...
        output_index = None
        output_bound_list = [
            bound.data for bound in output_snax_stride_pattern.upper_bounds
        ]
        ...
        ), "Output stride pattern must contain vector length"
        # Bring the last upper bound and temporal stride to the front
        output_snax_stride_pattern = StridePattern(
            [
                bound.data
                for i, bound in enumerate(output_snax_stride_pattern.upper_bounds)
                if i == output_index
            ]
            + [
                bound.data
            ...
            [
                stride.data
                for i, stride in enumerate(output_snax_stride_pattern.temporal_strides)
                if i == output_index
            ]
            + [
                stride.data
        ...
        output_stride_list = [
            stride.data for stride in output_snax_stride_pattern.temporal_strides
        ]
        ...
        assert (
            output_index is not None
        ), "Output stride pattern must contain triple repeat stride"
