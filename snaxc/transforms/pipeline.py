...
from snaxc.dialects.pipeline import DoubleStageOp, PipelineOp, StageOp
...
class PipelinePass(ModulePass):
    ...
    def apply(self, ctx: MLContext, op: builtin.ModuleOp) -> None:
        ...
        PatternRewriteWalker(
            DuplicateBuffers(), apply_recursively=False
        ).rewrite_module(op)
        PatternRewriteWalker(UnrollPipeline(), apply_recursively=False).rewrite_module(
            op
        )
        PatternRewriteWalker(UndoubleStages(), apply_recursively=False).rewrite_module(
            op
        )
        PatternRewriteWalker(DestructStages(), apply_recursively=False).rewrite_module(
            op
        )
        PatternRewriteWalker(RemovePipeline(), apply_recursively=False).rewrite_module(
            op
        )
