...
class CombineRescaleRewriter(RewritePattern):
    ...
    def match_and_rewrite(
        ...
        if isinstance(rescale_op.input.owner, Operation):
            if isinstance(rescale_op.input.owner, rescale_type):
                ...
                if isinstance(rescale_op, tosa.RescaleOp):
                    ...
                    new_rescale_op = kernel.RescaleOp(
                        input=new_input,
                        ...
                        shift=[new_shift],
                        max_int=rescale_op.max_int.value.data,
                        min_int=rescale_op.min_int.value.data,
                        double_round=True,
                    )
...
class CombineRescale(ModulePass):
    name = "combine-rescale"

    def apply(self, ctx: Context, op: builtin.ModuleOp) -> None:
        PatternRewriteWalker(
            CombineRescaleRewriter(),
