...
def generate_traces(folder: str, binary: str):
    ...
    traces = [f"results/tiled_add/{folder}/trace_hart_{i:05}.dasm" for i in range(2)]
    logs = [
    ...
    subprocess.check_output(
        [
            "python3",
            "../../benchmark/trace_to_perfetto.py",
            "--traces",
            *traces,
            "-i",
            *logs,
            "--elf",
            f"results/tiled_add/{folder}/{binary}",
            "--addr2line",
            "llvm-addr2line",
            "-o",
            f"{folder}_events.json",
        ]
    )
